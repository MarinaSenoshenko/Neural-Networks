...
def load_data(data_file_name: str = DATA_FILE) -> (tuple[np.ndarray, np.ndarray],
...
def back_propagate(neurons: tuple[np.ndarray, ...], target: np.ndarray,
                   outputs_buffer: tuple[np.ndarray, ...], weight_corrections_buffer: tuple[np.ndarray]) -> None:
...
def feed_forward(layers: tuple[np.ndarray, ...], data: np.ndarray, outputs: tuple[np.ndarray, ...]) -> None:
    copy_to(outputs[0], data)
    for i, layer in enumerate(layers):
        copy_to(outputs[i + 1], apply_layer(layer, append_ones_column(outputs[i])))


def learn(neurons: tuple[np.ndarray, ...], train_data: np.ndarray, train_target: np.ndarray,
          learning_rate: float, outputs_buffer: tuple[np.ndarray, ...], weight_corr_buffer: tuple[np.ndarray]) -> None:
    layers_count = len(neurons)
